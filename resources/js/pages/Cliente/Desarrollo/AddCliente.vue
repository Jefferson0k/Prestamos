...
// Obtener el sistema de toast utilizando el hook
...
// Método para manejar la navegación con enter
const handleKeydown = (event: KeyboardEvent, nextFieldId: string | null) => {
  if (event.key === "Enter") {
    event.preventDefault(); // Evitar que se envíe el formulario

    // Si hay un siguiente campo, enfocar en él
    if (nextFieldId) {
...
const submitForm = async () => {
  ...
  try {
    ...
    // Mostrar mensaje de éxito con la implementación correcta del toast
    toast({
      title: "¡Éxito!",
    ...
    if (error.response && error.response.data && error.response.data.errors) {
      ...
      // Error general utilizando el toast destructivo
      toast({
    ...
    >
      ...
      <form @submit.prevent="submitForm">
        <div class="grid grid-cols-2 gap-4 py-4 overflow-y-auto px-6 max-h-[60vh]">
          ...
          <div class="grid gap-2">
            <Label for="apellidos"
              >Apellidos <span class="text-red-500 text-sm">*</span></Label
            >
            <Input
              id="apellidos"
              type="text"
          ...
          <div class="grid gap-2">
            <Label for="telefono"
              >Nº Teléfono <span class="text-red-500 text-sm">*</span></Label
            >
            <Input
              id="telefono"
              type="number"
          ...
          <div class="grid gap-2">
            <Label for="direccion"
              >Dirección <span class="text-red-500 text-sm">*</span></Label
            >
            <Input
              id="direccion"
              type="text"
          ...
          <div class="grid gap-2">
            <Label for="correo"
              >Email address <span class="text-red-500 text-sm">*</span></Label
            >
            <Input
              id="correo"
              type="email"
          ...
          <div class="grid gap-2">
            <Label for="centro_trabajo"
              >Centro de Trabajo <span class="text-red-500 text-sm">*</span></Label
            >
            <Input
              id="centro_trabajo"
              type="text"
              v-model="form.centro_trabajo"
              placeholder="Centro Trabajo"
              @keydown="(e) => handleKeydown(e, 'foto')"
            />
        ...
        <DialogFooter class="p-6 pt-0 flex justify-end">
          <Button type="button" variant="outline" class="mr-2" @click="isOpen = false"
            >Cancelar</Button
          >
